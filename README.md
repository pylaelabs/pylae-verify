...
> end to end against real `pylae chain export` bundles: a 65-leaf / 2-block demo export and a
...
- **Merkle blocks** — recomputes each sealed block's Merkle root and the `actions_count`-bound
  block root.
- **Content-addressed config** — confirms every archived configuration re-hashes to its own
  address: the effective rule set, the CVE feed, and the rule manifest in both the forms a
  ...
  It also reads the **anchors out of the chain**, not out of the archive: a signed
  `compliance.config_active` leaf states which hashes a complete archive would hold, so a row
  that was quietly dropped is reported instead of being invisible. An anchor with no row is
  reported and is not a verdict — the specification draws that line and this follows it.
